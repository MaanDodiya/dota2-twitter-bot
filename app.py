...
def get_matchDetails():
    ...
    if(recentJSON==False):
        ...
        for match in recentJSON:
            ...
            link = "https://api.opendota.com/api/matches/" + str(match["match_id"])
            source = requests.get(link, headers={'User-agent': 'your bot 0.1'}).text
            soup = BeautifulSoup(source, 'lxml')
            DataString = soup.find('body').text
            DataJSON = json.loads(DataString)
            ...
            for i in range(10):
                if(DataJSON["players"][i]["player_slot"]<128):
                    tweetData["radiant_heroes"].append(DataJSON["players"][i]["hero_id"])
                else:
                    tweetData["dire_heroes"].append(DataJSON["players"][i]["hero_id"])
    ...
    init_log(tweets[0]["match_id"],mmr)
    return tweets

def getRank():
        tier = ["Herald","Guardian","Crusader","Archon","Legend","Ancient","Divine","Immortal"]
        link = "https://api.opendota.com/api/players/" + str(__DOTA2_ID)
        source = requests.get(link, headers={'User-agent': 'your bot 0.1'}).text
        soup = BeautifulSoup(source, 'lxml')
        DataString = soup.find('body').text
        DataJSON = json.loads(DataString)
        rank = tier[int((DataJSON["rank_tier"]/10)-1)] + "-" + str(DataJSON["rank_tier"]%10)
        return rank
...
def compileTweet():
    ...
    for match in tweetData:
        ...
        tweet += "[" + str(match["time"].day) + "/" + str(match["time"].month) + "/" + str(match["time"].year) + " "
        tweet += str(match["time"].hour//10) + str(match["time"].hour%10) + ":"
        tweet += str(match["time"].minute//10) + str(match["time"].minute%10) + ":"
        tweet += str(match["time"].second//10) + str(match["time"].second%10) + "(+5:30 GMT)" + "]\n"
        ...
        tweet += ", Duration: " + str(match["duration"]//60) + ":" + str((match["duration"]%60)//10) + str((match["duration"]%60)%10) + "\n"
        rad = []
        dire = []
        for i in range(5):
        ...
        tweet += "Radiant: " + rad[0] + ", " + rad[1] + ", " + rad[2] + ", " + rad[3] + ", " + rad[4] + "\n"
        tweet += "Dire: " + dire[0] + ", " + dire[1] + ", " + dire[2] + ", " + dire[3] + ", " + dire[4]
